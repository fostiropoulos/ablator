...
def test_assert_cat_attributes(capture_logger, capture_output,parallel_config, mock_experiment_directory):
    ...
    with patch.object(results, 'data', mock_df):
        ...
        stdout, stderr = capture_output(lambda: results._assert_cat_attributes(["attr2"]))
...
def test_read_result_exception_handling(tmp_path):
    ...
    config_content = 'experiment_dir: "\\tmp\\results\\experiment_8925_9991"\ndevice: cpu'
    ...
    with patch.object(ConfigBase, "load", side_effect=Exception("Mocked exception for testing")):
