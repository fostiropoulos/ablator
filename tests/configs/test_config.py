...
annotations = {
    ...
    "c2": Annotation(
        state=Stateful, optional=False, collection=Type, variable_type=TestConfig
    ),
}
...
def test_attrs(tmp_dir: Path):
    ...
    p.write(tmp_dir.joinpath("test.yaml"))
    loaded_p = p.load(tmp_dir.joinpath("test.yaml"))
